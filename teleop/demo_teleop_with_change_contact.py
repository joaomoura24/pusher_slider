...
class Slider:
    ...
    def near_point(self, point):
        ...
        return self.pos + R@pos_in_slider_frame, pos_in_slider_frame, side, in_contact
...
def main():
    ...
    stick_pusher_to_slider = True
    prev_trigger = None
    num_trigger_switches = 0
    running = True
    ...
    try:
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if (event.key == pygame.K_LEFT) or (event.key == pygame.K_RIGHT):
                        ...
                        stick_pusher_to_slider = True
                        find_side_to_stick_to = True
                elif event.type == pygame.JOYBUTTONUP:
                    if event.button == 0:
                        find_side_to_stick_to = True


            if find_side_to_stick_to:
                pusher_point = Point(pusher[0], pusher[1], slider)
                _, pusher, side, _ = slider.near_point(pusher_point)
            ...
            if use_joy:
                joy.reset()
                j = joy.get_axes()
                trigger = bool(joy.get_button(0))
                # if trigger != prev_trigger:
                #     num_trigger_switches += 1
                #     if num_trigger_switches == 2:
                #         # accept switch
                #         trigger_use = trigger
                #         num_trigger_switches = 0
                #     prev_trigger = trigger
                stick_pusher_to_slider = not trigger

            # Setup
            curr_time = time.time()
            old_pusher = pusher.copy()
            if stick_pusher_to_slider:
                if side == 'bottom':
                    ...
                    raise ValueError(f'side is not recognized ({side})')
            ...
            if not stick_pusher_to_slider:
                R = Rotation.from_euler('z', slider.heading, degrees=False).as_matrix()[:2,:2]
                pusher_in_world = R@pusher + slider.pos
                pusher_in_world[0] += max_pusher_vel*dt*j[0]
                pusher_in_world[1] += -max_pusher_vel*dt*j[1]

                pusher = R.T@(pusher_in_world - slider.pos)
    ...
    if save_data:
        ...
        stamp = time.time_ns()
        fn = os.path.join('data', 'data_change_contact_%d.csv'%stamp)
        pandas.DataFrame(data).to_csv(fn)
        print("Saved", fn)

        # Save configuration data
        fn = os.path.join('data', 'data_change_contact_%d.config'%stamp)
