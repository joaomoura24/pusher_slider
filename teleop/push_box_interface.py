...
env_filename = sys.argv[1]  # assume path to environment specification
with open(env_filename, 'rb') as env_file:
    env_config = pickle.load(env_file)
...
class Data:
    ...
    def save(self):
        ...
        u_pusher = numpy.array(self.u_pusher).T
        u_point = numpy.array(self.u_point).T
        ...
        print("Saved", filename, 'in interface_data/')
...
pi = numpy.pi
xB0 = env_config['start'][0]  # initial x-position of body in world
yB0 = env_config['start'][1]  # initial y-position of body in world
...
goalX = env_config['goal'][0]  # x-position of goal of body
goalY = env_config['goal'][1]  # y-position of goal of body
...
# tunnel_center_shift = numpy.array([0, tunnel_yshift]) # numpy.array([0, -0.2]) #numpy.array([0, -0.3])
...
for ox, oy, oR in env_config['obs']:
    screen.windows['robotenv'].static_circle(
        'black',
        screen.windows['robotenv'].convert_position((ox, oy)),
        screen.windows['robotenv'].convert_scalar(oR)
    )
...
data.u_pusher.append(numpy.zeros(3).tolist())
data.u_point.append(numpy.zeros(2).tolist())
...
while running:
    ...
    if dynamics_model == DYN_POINT_MODEL:
        ...
        u_point = numpy.asarray(h)
        u_pusher = numpy.zeros(3)
        xdot = dyn_point(x, h)
        in_contact = False
        ...
        u_point = numpy.zeros(2)
        u_pusher = numpy.asarray(h)
    ...
    data.u_pusher.append(u_pusher.tolist())
    data.u_point.append(u_point.tolist())
